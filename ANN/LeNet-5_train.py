...
for E in range(epoch):
    ...
    for i in range(train_images.shape[0] // batch_size):
        ...
        for j in range(batch_size):
            labels[j][label[j]] = 1
        loss, prediction = net.forward(img, labels, is_train=True)  # 训练阶段
    ...
    for k in range(test_images.shape[0] // test_batch):
        ...
        labels = np.zeros((test_batch, 10))
        for j in range(test_batch):
            labels[j][label[j]] = 1
        _, prediction = net.forward(img, labels, is_train=False)  # 测试阶段
